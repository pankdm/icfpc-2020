...
lazy_static! {
    static ref SYMBOLS_LIST: Vec<(i64, &'static str)> = vec![
        (0, "ap"),
        (12, "=="),
        (417, "inc"),
        (401, "dec"),
    ...
    static ref SYMBOLS: HashMap<i64, &'static str> = SYMBOLS_LIST
    .iter()
    .copied()
    .collect();
}
        ...
        for cy in 0..(delta - 1) {
            for cx in 0..(delta - 1) {
                if image[x + 1 + cx][y + 1 + cy] == set {
                    final_value += 1 << (cy * (delta - 1) + cx) as i64;
                }
            }
        }
...
fn encode_symbol(value: i64) -> Image {
    assert!(value >= 0);
    // println!("Encoding {}", value);
    let ln = if value == 0 {
        1.0
    } else {
        ((value + 1) as f64).log2().ceil()
    };
