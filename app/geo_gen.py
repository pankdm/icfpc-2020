...
def check_geo(coord, speed, accel):
    ...
    for _ in range(num_ticks):
        if abs(state.pos[0]) <= 20 and abs(state.pos[1]) <= 20:
            return False
        state = state.update()
    return True

def main():
    coords = [(x, y) for (x, y) in itertools.product(range(-48, 49), repeat=2) if abs(x) > 20 or abs(y) > 20]
    speeds = [(x, y) for (x, y) in itertools.product(range(-10, 11), repeat=2) if abs(x) + abs(y) < 8]
